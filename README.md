...
1.  **克隆仓库**
    ```bash
    git clone https://github.com/fastnas2023/Nebula-meeting.git
    cd Nebula-meeting
    ```
...
WebRTC UI 交互逻辑梳理文档请参阅 [WEBRTC_UI_SPEC.md](WEBRTC_UI_SPEC.md)。

## WebRTC 房间列表清理规则

- 默认规则：当某个会议房间无人在线后，保留 120 分钟；超时将自动从“活跃房间”列表中清理（服务端内存释放）。
- 可配置：服务端环境变量 `ROOM_IDLE_TTL_MS`（毫秒），以及清理扫描间隔 `ROOM_CLEANUP_INTERVAL_MS`（毫秒）。
